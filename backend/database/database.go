...
func (db *Database) CreatePoll(poll *models.Poll) error {
	...
	err = tx.QueryRow(
		context.Background(),
		`INSERT INTO polls (title, description, deadline) VALUES ($1, $2, $3) RETURNING id, created_at`,
...
func (db *Database) GetPollByID(id int) (*models.Poll, error) {
	...
	err := db.pool.QueryRow(
		...
		`SELECT p.id, p.title, p.description, p.deadline, p.created_at,
		 ...
		 GROUP BY p.id`,
		id,
	).Scan(&poll.ID, &poll.Title, &poll.Description, &poll.Deadline, &poll.CreatedAt, &poll.TotalVotes)
...
func (db *Database) Vote(pollID int, optionID int, voterID string) error {
	tx, err := db.pool.Begin(context.Background())
	if err != nil {
		return err
	}
